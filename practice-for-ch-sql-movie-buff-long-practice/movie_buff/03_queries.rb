def what_was_that_one_with(those_actors)
  ...
  Movie.joins(:actors).where("actors.name": those_actors).group(:id).having("COUNT(movies.id) = ?", those_actors.length).select(:title,:id)
...
def golden_age
  ...
  Movie.select("yr/10 AS decade").group("yr/10").order("AVG(score) DESC").limit(1).first.decade * 10
end
...
def costars(name)
  ...
  subquery.joins(:actors).where("actors.name != ?", name).group("actors.name")
end
