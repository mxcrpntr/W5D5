...
def harrison_ford
  ...
  # as a lead actor.
  Actor.joins(:movies).select("movies.id,movies.title").where("actors.name = 'Harrison Ford' AND castings.ord != 1").group("movies.id")
end
...
def biggest_cast
  ...
  # actors).
  Movie.joins(:actors).select(:id,:title).group(:id).order("COUNT(movies.id) DESC").limit(3)
end
...
def directed_by_one_of(them)
  ...
  Actor.joins(:directed_movies).where(name: them).select("movies.id,movies.title")
end
